...
public:
  ...
  std::unique_ptr<Runtime::ImportObject> create() const noexcept {
    assuming(Desc);
    return std::unique_ptr<Runtime::ImportObject>(Desc->Create());
  }
...
public:
  ...
  void registerOptions(PO::ArgumentParser &Parser) noexcept {
    assuming(Desc);
