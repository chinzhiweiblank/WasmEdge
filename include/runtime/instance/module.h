...
private:
  ...
  using StoreCallback = void(StoreManager *Store, const ModuleInstance *Mod);
  void linkStore(StoreManager *Store, StoreCallback Callback) {
  ...
  /// Linked store.
  std::map<StoreManager *, std::function<StoreCallback>> LinkedStore;
};
